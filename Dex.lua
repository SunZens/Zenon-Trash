...
local Ignoran, Properties, ScriptViewer, Notebook -- Major Apps
...
local function main()
  local Ignoran = {}
  local tree,listEntries,ignoranOrders,searchResults,specResults = {},{},{},{},{}
  local expanded
  ...
  addObject = function(root)
    ...
    if sortingEnabled and expanded[par] and par.Sorted then
      ...
      local sorter = Ignoran.NodeSorter
    ...
    if not updateDebounce and Ignoran.IsNodeVisible(par) then
      if expanded[par] then
        Ignoran.PerformUpdate()
      elseif not refreshDebounce then
        Ignoran.PerformRefresh()
      end
    end
  end
  ...
  removeObject = function(root)
    ...
    if par and not updateDebounce and Ignoran.IsNodeVisible(par) then
      if expanded[par] then
        Ignoran.PerformUpdate()
      elseif not refreshDebounce then
        Ignoran.PerformRefresh()
  ...
  moveObject = function(obj)
    ...
    if not updateDebounce and (Ignoran.IsNodeVisible(newPar) or Ignoran.IsNodeVisible(oldPar)) then
  ...
  Ignoran.ViewWidth = 0
  Ignoran.Index = 0
  Ignoran.EntryIndent = 20
  Ignoran.FreeWidth = 32
  Ignoran.GuiElems = {}
  
  Ignoran.InitRenameBox = function()
    ...
    renameBox.Parent = Ignoran.Window.GuiElems.Content.List
  ...
  Ignoran.SetRenamingNode = function(node)
    renamingNode = node
  ...
  Ignoran.SetSortingEnabled = function(val)
    sortingEnabled = val
    Settings.Ignoran.Sorting = val
  end

  Ignoran.UpdateView = function()
    local maxNodes = math.ceil(treeFrame.AbsoluteSize.Y / 20)
    local maxX = treeFrame.AbsoluteSize.X
    local totalWidth = Ignoran.ViewWidth + Ignoran.FreeWidth
    ...
    if oldSize ~= treeFrame.Size then
      ...
      if scrollV.Gui.Visible and scrollH.Gui.Visible then
        ...
        Ignoran.Window.GuiElems.Content.ScrollCorner.Visible = true
      else
        scrollV.Gui.Size = UDim2.new(0,16,1,-23)
        scrollH.Gui.Size = UDim2.new(1,0,0,16)
        Ignoran.Window.GuiElems.Content.ScrollCorner.Visible = false
      end

      Ignoran.Index = scrollV.Index
    end
  end

  Ignoran.NodeSorter = function(a,b)
    ...
    local aOrder = ignoranOrders[aClass]
    local bOrder = ignoranOrders[bClass]
    if not aOrder then aOrder = RMD.Classes[aClass] and tonumber(RMD.Classes[aClass].IgnoranOrder) or 9999 ignoranOrders[aClass] = aOrder end
    if not bOrder then bOrder = RMD.Classes[bClass] and tonumber(RMD.Classes[bClass].IgnoranOrder) or 9999 ignoranOrders[bClass] = bOrder end
  ...
  Ignoran.Update = function()
    table.clear(tree)
    local maxNameWidth,maxDepth,count = 0,1,1
    local nameCache = {}
    local font = Enum.Font.SourceSans
    local size = Vector2.new(math.huge,20)
    local useNameWidth = Settings.Ignoran.UseNameWidth
    local tSort = table.sort
    local sortFunc = Ignoran.NodeSorter
    local isSearching = (expanded == Ignoran.SearchExpanded)
    ...
    Ignoran.MaxNameWidth = maxNameWidth
    Ignoran.MaxDepth = maxDepth
    Ignoran.ViewWidth = useNameWidth and Ignoran.EntryIndent*maxDepth + maxNameWidth + 26 or Ignoran.EntryIndent*maxDepth + 226
    Ignoran.UpdateView()
  end

  Ignoran.StartDrag = function(offX,offY)
    if Ignoran.Dragging then return end
    ...
    local newGui = Instance.new("ScreenGui",game.Players.LocalPlayer:WaitForChild("PlayerGui"))
    newGui.DisplayOrder = Main.DisplayOrders.Menu
    newGui.Parent = game:GetService("Players").LocalPlayer.PlayerGui
  ...
  Ignoran.NewListEntry = function(index)
    ...
    newEntry.InputBegan:Connect(function(input)
      local node = tree[index + Ignoran.Index]
        ...
        mouseEvent = service.UserInputService.InputChanged:Connect(function(input)
          if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            ...
            if dist > 5 then
              ...
              mouseEvent:Disconnect()
              isRenaming = false
              Ignoran.StartDrag(listOffsetX, listOffsetY)
            end
          end
        end)
      end
    ...
    newEntry.Indent.Expand.InputBegan:Connect(function(input)
      ...
      if input.UserInputType == Enum.UserInputType.Touch then
        Ignoran.MiscIcons:DisplayByKey(newEntry.Indent.Expand.Icon, expanded[node] and "Collapse_Over" or "Expand_Over")
    ...
    newEntry.Indent.Expand.InputEnded:Connect(function(input)
      ...
      if input.UserInputType == Enum.UserInputType.Touch then
        Ignoran.MiscIcons:DisplayByKey(newEntry.Indent.Expand.Icon, expanded[node] and "Collapse" or "Expand")
      elseif input.UserInputType == Enum.UserInputType.MouseMovement then
        Ignoran.MiscIcons:DisplayByKey(newEntry.Indent.Expand.Icon, expanded[node] and "Collapse" or "Expand")
      end
    end)
  ...
  Ignoran.Refresh = function()
    ...
    for i = 1,maxNodes do
      local entry = listEntries[i]
      if not listEntries[i] then entry = Ignoran.NewListEntry(i) listEntries[i] = entry Ignoran.ClickSystem:Add(entry) end
      
      local node = tree[i + Ignoran.Index]
      if node then
        local obj = node.Obj
        local depth = Ignoran.EntryIndent*Ignoran.NodeDepth(node)

        entry.Visible = true
        entry.Position = UDim2.new(0,-scrollH.Index,0,entry.Position.Y.Offset)
        entry.Size = UDim2.new(0,Ignoran.ViewWidth,0,20)
        ...
        entry.Indent.EntryName.TextTruncate = (Settings.Ignoran.UseNameWidth and Enum.TextTruncate.None or Enum.TextTruncate.AtEnd)
        
        Ignoran.MiscIcons:DisplayIgnoranIcons(entry.Indent.Icon, obj.ClassName)
        ...
        if #node > 0 and expanded[node] ~= 0 then
          if Lib.CheckMouseInGui(entry.Indent.Expand) then
            Ignoran.MiscIcons:DisplayByKey(entry.Indent.Expand.Icon, expanded[node] and "Collapse_Over" or "Expand_Over")
          else
            Ignoran.MiscIcons:DisplayByKey(entry.Indent.Expand.Icon, expanded[node] and "Collapse" or "Expand")
          end
    ...
    for i = maxNodes+1, #listEntries do
      Ignoran.ClickSystem:Remove(listEntries[i])
      listEntries[i]:Destroy()
      listEntries[i] = nil
    end
  end

  Ignoran.PerformUpdate = function(instant)
    updateDebounce = true
    Lib.FastWait(not instant and 0.1)
    if not updateDebounce then return end
    updateDebounce = false
    if not Ignoran.Window:IsVisible() then return end
    Ignoran.Update()
    Ignoran.Refresh()
  end

  Ignoran.ForceUpdate = function(norefresh)
    updateDebounce = false
    Ignoran.Update()
    if not norefresh then Ignoran.Refresh() end
  end

  Ignoran.PerformRefresh = function()
    refreshDebounce = true
    Lib.FastWait(0.1)
    refreshDebounce = false
    if updateDebounce or not Ignoran.Window:IsVisible() then return end
    Ignoran.Refresh()
  end

  Ignoran.IsNodeVisible = function(node)
    if not node then return end
  ...
  Ignoran.NodeDepth = function(node)
    local depth = 0
  ...
  Ignoran.SetupConnections = function()
    ...
    if Settings.Ignoran.UseNameWidth then
  ...
  Ignoran.ViewNode = function(node)
    if not node then return end

    Ignoran.MakeNodeVisible(node)
    Ignoran.ForceUpdate(true)
    local visibleSpace = scrollV.VisibleSpace

    for i,v in next,tree do
      if v == node then
        local relative = i - 1
        if Ignoran.Index > relative then
          scrollV.Index = relative
        elseif Ignoran.Index + visibleSpace - 1 <= relative then
          scrollV.Index = relative - visibleSpace + 2
        end
      end
    end

    scrollV:Update() Ignoran.Index = scrollV.Index
    Ignoran.Refresh()
  end

  Ignoran.ViewObj = function(obj)
    Ignoran.ViewNode(nodes[obj])
  end

  Ignoran.MakeNodeVisible = function(node,expandRoot)
    if not node then return end
    ...
    if hasExpanded and not updateDebounce then
      coroutine.wrap(Ignoran.PerformUpdate)(true)
    end
  end

  Ignoran.ShowRightClick = function(MousePos)
    local Mouse = MousePos or Main.Mouse
    local context = Ignoran.RightClickContext
    ...
    context:AddDivider()
    if expanded == Ignoran.SearchExpanded then context:AddRegistered("CLEAR_SEARCH_AND_JUMP_TO") end
   ...
   -- context:AddRegistered("VIEW_CONNECTIONS")
    ...
    Ignoran.LastRightClickX, Ignoran.LastRightClickY = Mouse.X, Mouse.Y
    context:Show(Mouse.X, Mouse.Y)
  end
  
  Ignoran.InitRightClick = function()
    local context = Lib.ContextMenu.new()
    
    context:Register("CUT",{Name = "ตัด", IconMap = Ignoran.MiscIcons, Icon = "Cut", DisabledIcon = "Cut_Disabled", Shortcut = "Ctrl+Z", OnClick = function()
    ...
    context:Register("COPY",{Name = "คัดลอก", IconMap = Ignoran.MiscIcons, Icon = "Copy", DisabledIcon = "Copy_Disabled", Shortcut = "Ctrl+C", OnClick = function()
    ...
    context:Register("PASTE",{Name = "วางใส่ข้างใน", IconMap = Ignoran.MiscIcons, Icon = "Paste", DisabledIcon = "Paste_Disabled", Shortcut = "Ctrl+Shift+V", OnClick = function()
      local sList = selection.List
      local newSelection = {}
      local count = 1
      for i = 1,#sList do
        local node = sList[i]
        local inst = node.Obj
        Ignoran.MakeNodeVisible(node,true)
      ...
      if #newSelection > 0 then
        Ignoran.ViewNode(newSelection[1])
      end
    end})
    
    context:Register("DUPLICATE",{Name = "ทำซ้ำ", IconMap = Ignoran.MiscIcons, Icon = "Copy", DisabledIcon = "Copy_Disabled", Shortcut = "Ctrl+D", OnClick = function()
      local clone = game.Clone
      local sList = selection.List
      local newSelection = {}
      local count = 1
    ...
    context:Register("DELETE",{Name = "ลบ", IconMap = Ignoran.MiscIcons, Icon = "Delete", DisabledIcon = "Delete_Disabled", Shortcut = "Del", OnClick = function()
    ...
    context:Register("RENAME",{Name = "แก้ไขชื่อ", IconMap = Ignoran.MiscIcons, Icon = "Rename", DisabledIcon = "Rename_Disabled", Shortcut = "F2", OnClick = function()
      local sList = selection.List
      if sList[1] then
        Ignoran.SetRenamingNode(sList[1])
      end
    end})

    context:Register("GROUP",{Name = "กรุ๊ป", IconMap = Ignoran.MiscIcons, Icon = "Group", DisabledIcon = "Group_Disabled", Shortcut = "Ctrl+G", OnClick = function()
      local sList = selection.List
      ...
      if nodes[model] then
        ...
        Ignoran.ViewNode(nodes[model])
      end
    end})

    context:Register("UNGROUP",{Name = "ยกเลิกกรุ๊ป", IconMap = Ignoran.MiscIcons, Icon = "Ungroup", DisabledIcon = "Ungroup_Disabled", Shortcut = "Ctrl+U", OnClick = function()
      local newSelection = {}
      local count = 1
      local isa = game.IsA
    ...
    context:Register("SELECT_CHILDREN",{Name = "เลือกไฟล์ Children", IconMap = Ignoran.MiscIcons, Icon = "SelectChildren", DisabledIcon = "SelectChildren_Disabled", OnClick = function()
      local newSelection = {}
      local count = 1
      local sList = selection.List
      ...
      for i = 1,#sList do
        ...
        for ind = 1,#node do
          ...
          if ind == 1 then Ignoran.MakeNodeVisible(cNode) end
    ...
    context:Register("JUMP_TO_PARENT",{Name = "ไปหา Parent", IconMap = Ignoran.MiscIcons, Icon = "JumpToParent", OnClick = function()
      local newSelection = {}
      local count = 1
      local sList = selection.List
    ...
    context:Register("TELEPORT_TO",{Name = "เทเลพอร์ตไปหา", IconMap = Ignoran.MiscIcons, Icon = "TeleportTo", OnClick = function()
      local sList = selection.List
      ...
      for _,node in next, sList do
        ...
        if Obj:IsA("BasePart") then
          if Obj.CanCollide then
            ...
            plrRP.CFrame = CFrame.new(Obj.Position + Settings.Ignoran.TeleportToOffset)
          end
          break
        elseif Obj:IsA("Model") then
          if Obj.PrimaryPart then
            if Obj.PrimaryPart.CanCollide then
              ...
              plrRP.CFrame = CFrame.new(Obj.PrimaryPart.Position + Settings.Ignoran.TeleportToOffset)
            end
            break
          else
            ...
            if part and nodes[part] then
              if part.CanCollide then
                ...
                plrRP.CFrame = CFrame.new(part.Position + Settings.Ignoran.TeleportToOffset)
              end
              break
    ...
    local OldAnimation
    context:Register("PLAY_TWEEN",{Name = "เล่น Tween", IconMap = Ignoran.MiscIcons, Icon = "Play", OnClick = function()
      local sList = selection.List
    ...
    local OldAnimation
    context:Register("LOAD_ANIMATION",{Name = "เล่นอนิเมชั่น", IconMap = Ignoran.MiscIcons, Icon = "Play", OnClick = function()
      local sList = selection.List
    ...
    context:Register("STOP_ANIMATION",{Name = "หยุดอนิเมชั่น", IconMap = Ignoran.MiscIcons, Icon = "Pause", OnClick = function()
      local sList = selection.List
    ...
    context:Register("COPY_PATH",{Name = "คัดลอกเส้นทางไฟล์", IconMap = Ignoran.ClassIcons, Icon = 50, OnClick = function()
      local sList = selection.List
      if #sList == 1 then
        env.setclipboard(clth(Ignoran.GetInstancePath(sList[1].Obj)))
      elseif #sList > 1 then
        local resList = {"{"}
        local count = 2
        for i = 1,#sList do
          local path = "\t"..clth(Ignoran.GetInstancePath(sList[i].Obj))..","
          if #path > 0 then
            resList[count] = path
            count = count+1
          end
    ...
    context:Register("INSERT_OBJECT",{Name = "เพิ่ม Object", IconMap = Ignoran.MiscIcons, Icon = "InsertObject", OnClick = function()
      local mouse = Main.Mouse
      local x,y = Ignoran.LastRightClickX or mouse.X, Ignoran.LastRightClickY or mouse.Y
      Ignoran.InsertObjectContext:Show(x,y)
    end})

    context:Register("CALL_FUNCTION",{Name = "เรียกฟังก์ชั่น", IconMap = Ignoran.ClassIcons, Icon = 66, OnClick = function()
      
    end})

    context:Register("GET_REFERENCES",{Name = "ดู Lua References", IconMap = Ignoran.ClassIcons, Icon = 34, OnClick = function()
      
    end})
    
    context:Register("SAVE_INST",{Name = "เซฟไปที่ไฟล์", IconMap = Ignoran.MiscIcons, Icon = "Save", OnClick = function()
    ...
    context:Register("VIEW_API",{Name = "ดูหน้า API", IconMap = Ignoran.MiscIcons, Icon = "Reference", OnClick = function()
      
    end})
    
    context:Register("VIEW_OBJECT",{Name = "ดู Object (คลิ๊กขวาเพื่อรีเซ็ต)", IconMap = Ignoran.ClassIcons, Icon = 5, OnClick = function()
      local sList = selection.List
      local isa = game.IsA
    ...
    context:Register("FIRE_TOUCHTRANSMITTER",{Name = "ยิง TouchTransmitter", IconMap = Ignoran.ClassIcons, Icon = 37, OnClick = function()
    ...
    context:Register("FIRE_CLICKDETECTOR",{Name = "ยิง ClickDetector", IconMap = Ignoran.ClassIcons, Icon = 41, OnClick = function()
      local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
      if not hrp then return end
      for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ClickDetector") then fireclickdetector(v.Obj) end end
    end})

    context:Register("FIRE_PROXIMITYPROMPT",{Name = "ยิง ProximityPrompt", IconMap = Ignoran.ClassIcons, Icon = 124, OnClick = function()
      local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
      if not hrp then return end
      for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ProximityPrompt") then fireproximityprompt(v.Obj) end end
    end})

    context:Register("VIEW_SCRIPT",{Name = "ดูสคริปต์", IconMap = Ignoran.MiscIcons, Icon = "ViewScript", OnClick = function()
      local scr = selection.List[1] and selection.List[1].Obj
      if scr then ScriptViewer.ViewScript(scr) end
    end})
    
    context:Register("SAVE_BYTECODE",{Name = "เซฟ ScriptBytecode ลงในไฟล์", IconMap = Ignoran.MiscIcons, Icon = "Save", OnClick = function()
    ...
    context:Register("SELECT_CHARACTER",{Name = "เลือก Character", IconMap = Ignoran.ClassIcons, Icon = 9, OnClick = function()
      local newSelection = {}
      local count = 1
      local sList = selection.List
      local isa = game.IsA
    ...
    context:Register("VIEW_PLAYER",{Name = "ดูผู้เล่น", IconMap = Ignoran.ClassIcons, Icon = 5, OnClick = function()
      local newSelection = {}
      local count = 1
      local sList = selection.List
      local isa = game.IsA
    ...
    context:Register("SELECT_LOCAL_PLAYER",{Name = "เลือก Local Player", IconMap = Ignoran.ClassIcons, Icon = 9, OnClick = function()
      pcall(function() if nodes[plr] then selection:Set(nodes[plr]) Ignoran.ViewNode(nodes[plr]) end end)
    end})
    
    context:Register("SELECT_ALL_CHARACTERS",{Name = "เลือก Character ทั้งหมด", IconMap = Ignoran.ClassIcons, Icon = 2, OnClick = function()
      local newSelection = {}
      local sList = selection.List
      
      for i,v in next, service.Players:GetPlayers() do
        if v.Character and nodes[v.Character] then
          if i == 1 then Ignoran.MakeNodeVisible(v.Character) end
    ...
    context:Register("REFRESH_NIL",{Name = "รีเฟรช Nil Instances", OnClick = function()
      Ignoran.RefreshNilInstances()
    end})
    
    context:Register("HIDE_NIL",{Name = "ซ่อน Nil Instances", OnClick = function()
      Ignoran.HideNilInstances()
    end})
    
    Ignoran.RightClickContext = context
  end

  Ignoran.HideNilInstances = function()
    table.clear(nilMap)
  ...
  Ignoran.RefreshNilInstances = function()
    ...
    --local newNilMap = {}
    --local newNilRoots = {}
    --local nilRoots = Ignoran.NilRoots
    --local connect = game.DescendantAdded.Connect
    --local disconnect
    --if not nilRoots then nilRoots = {} Ignoran.NilRoots = nilRoots end
    ...
    --nilMap = newNilMap
    --Ignoran.NilRoots = newNilRoots

    Ignoran.Update()
    Ignoran.Refresh()
  end

  Ignoran.GetInstancePath = function(obj)
    ...
    local useGetCh = Settings.Ignoran.CopyPathUseGetChildren
  ...
  Ignoran.InitInsertObject = function()
    local context = Lib.ContextMenu.new()
    ...
    local function defaultProps(obj)
      for class, props in pairs(Ignoran.DefaultProps) do
    ...
    for i = 1,#classes do
      ...
      local rmdEntry = RMD.Classes[class.Name]
      local iconInd = rmdEntry and tonumber(rmdEntry.IgnoranImageIndex) or 0
      ...
      context:Add({Name = class.Name, IconMap = Ignoran.ClassIcons, Icon = iconInd, OnClick = onClick})
    end
    
    Ignoran.InsertObjectContext = context
  end
  
  Ignoran._SearchFilters = {} do
    local Filters = Ignoran._SearchFilters
  ...
  Ignoran.DoSearch = function(query)
    table.clear(Ignoran.SearchExpanded)
    table.clear(searchResults)
    expanded = (#query == 0 and Ignoran.Expanded) or Ignoran.SearchExpanded
    
    local tostr = tostring;
    local tfind = table.find;
    
    local Filters = Ignoran._SearchFilters
    local expandTable = Ignoran.SearchExpanded
    ...
    --[=[if #query > 0 then
      ...
      local function defaultSearch(root)
        ...
        for i = 1,#root do
          ...
          elseif IgnoranSearch[lower(tostring(obj))] then
      ...
      if Main.Elevated then
        local start = tick()
        searchFunc,specFilters = Ignoran.BuildSearchFunc(query)
  ...
  Ignoran.ClearSearch = function()
    Ignoran.GuiElems.SearchBar.Text = ""
    expanded = Ignoran.Expanded
    searchFunc = nil
  end

  Ignoran.InitSearch = function()
    local TweenService = service.TweenService
    local SearchFrame = Ignoran.GuiElems.ToolBar.SearchFrame
    local searchBox = SearchFrame.SearchBox
    Ignoran.GuiElems.SearchBar = searchBox
    ...
    searchBox.FocusLost:Connect(function()
      Ignoran.DoSearch(searchBox.Text)
    end)
  end

  Ignoran.InitEntryTemplate = function()
    ...
    sys.OnDown:Connect(function(item,combo,button)
      ...
      local node = tree[ind + Ignoran.Index]
      if not node then return end
    ...
    sys.OnRelease:Connect(function(item,combo,button,position)
      ...
      if button == 1 then
        ...
        if combo == 1 and id == sys.ClickId and sys.IsRenaming and selection.Map[node] then
          Ignoran.SetRenamingNode(node)
        end
      elseif button == 2 then
        Ignoran.ShowRightClick(position)
      end
    end)
    Ignoran.ClickSystem = sys
  end

  Ignoran.InitDelCleaner = function()
    coroutine.wrap(function()
      ...
      while true do
        ...
        if processed and not refreshDebounce then Ignoran.PerformRefresh() end
        fw(0.5)
      end
    end)()
  end

  Ignoran.UpdateSelectionVisuals = function()
    local holder = Ignoran.SelectionVisualsHolder
    local isa = game.IsA
    local clone = game.Clone
    if not holder then
      holder = Instance.new("ScreenGui",game.Players.LocalPlayer:WaitForChild("PlayerGui"))
      holder.Name = "IgnoranSelections"
      holder.DisplayOrder = Main.DisplayOrders.Core
      Lib.ShowGui(holder)
      Ignoran.SelectionVisualsHolder = holder
      Ignoran.SelectionVisualCons = {}
      ...
      Ignoran.SelectionVisualGui = guiTemplate

      local boxTemplate = Instance.new("SelectionBox")
      boxTemplate.LineThickness = 0.03
      boxTemplate.Color3 = Color3.fromRGB(0, 170, 255)
      Ignoran.SelectionVisualBox = boxTemplate
    end
    holder:ClearAllChildren()
    
    -- Updates theme
    for i,v in pairs(Ignoran.SelectionVisualGui:GetChildren()) do
      v.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
    end
    
    local attachCons = Ignoran.SelectionVisualCons
    for i = 1,#attachCons do
      attachCons[i].Destroy()
    end
    table.clear(attachCons)

    local partEnabled = Settings.Ignoran.PartSelectionBox
    local guiEnabled = Settings.Ignoran.GuiSelectionBox
    if not partEnabled and not guiEnabled then return end

    local svg = Ignoran.SelectionVisualGui
    local svb = Ignoran.SelectionVisualBox
  ...
  Ignoran.Init = function()
    Ignoran.ClassIcons = Lib.IconMap.newLinear("rbxasset://textures/ClassImages.PNG", 16, 16)
    Ignoran.MiscIcons = Main.MiscIcons
    
    clipboard = {}
    
    selection = Lib.Set.new()
    selection.ShiftSet = {}
    selection.Changed:Connect(Properties.ShowIgnoranProps)
    Ignoran.Selection = selection
    
    Ignoran.InitRightClick()
    Ignoran.InitInsertObject()
    Ignoran.SetSortingEnabled(Settings.Ignoran.Sorting)
    Ignoran.Expanded = setmetatable({},{__mode = "k"})
    Ignoran.SearchExpanded = setmetatable({},{__mode = "k"})
    expanded = Ignoran.Expanded

    nilNode.Obj.Name = "Nil Instances"
    nilNode.Locked = true

    local ignoranItems = create({
      {1,"Folder",{Name="IgnoranItems",}},
    ...
    toolBar = ignoranItems.ToolBar
    treeFrame = ignoranItems.List

    Ignoran.GuiElems.ToolBar = toolBar
    Ignoran.GuiElems.TreeFrame = treeFrame
    ...
    scrollH.Increment = 5
    scrollH.WheelIncrement = Ignoran.EntryIndent
    ...
    window.OnDeactivate:Connect(function() Ignoran.Active = false end)
    window.OnMinimize:Connect(function() Ignoran.Active = false end)
    
    -- Settings
    autoUpdateSearch = Settings.Ignoran.AutoUpdateSearch
...
local function main()
  ...
  Properties.ComputeConflicts = function(p)
    ...
    local sList = Ignoran.Selection.List
...
local function main()
  ...
  ScriptViewer.ViewScript = function(scr)
    ...
    if not success or not source then source, PreviousScr = "-- Sun - Source failed to decompile", nil else PreviousScr = scr end
...
local function main()
  ...
  Lib.IconMap = (function()
    ...
    funcs.IgnoranIcons = { ["MapId"] = _MapId, ["Icons"] = _Icons }
    ...
    funcs.DisplayByKey = function(self, obj, key)
      if self.IndexDict[key] then
        ...
        Ignoran.ClassIcons:Display(obj, rmdEntry and rmdEntry.IgnoranImageIndex or 0)
      end
    end
    ...
    funcs.GetIgnoranIcon = function(self, obj, index)
      index = (self.IgnoranIcons.Icons[index] or 0)
    ...
    funcs.DisplayIgnoranIcons = function(self, Frame, index)
      if Frame:FindFirstChild("IconMap") then
        self:GetIgnoranIcon(Frame.IconMap, index)
      else
        Frame.ClipsDescendants = true
        
        local obj = Instance.new("ImageLabel", Frame)
        obj.BackgroundTransparency = 1
        obj.Image = ("http://www.roblox.com/asset/?id=" .. (self.IgnoranIcons.MapId))
        obj.Name = "IconMap"
        self:GetIgnoranIcon(obj, index)
      end
    end
  ...
  Lib.Window = (function()
    ...
    local createGui = function(self)
      local gui = create({
        {1,"ScreenGui",{Name="Window",Parent=game.Players.LocalPlayer:WaitForChild("PlayerGui"),}},
    ...
    static.Init = function()
      ...
      sidesGui = Instance.new("ScreenGui",game.Players.LocalPlayer:WaitForChild("PlayerGui"))
      ...
      alignIndicator = Instance.new("ScreenGui",game.Players.LocalPlayer:WaitForChild("PlayerGui"))
  ...
  Lib.ContextMenu = (function()
    ...
    local function createGui(self)
      local contextGui = create({
        {1,"ScreenGui",{DisplayOrder=1000000,Name="Context",ZIndexBehavior=1,Parent=game.Players.LocalPlayer:WaitForChild("PlayerGui"),}},
      ...
      for i = 1,#items do
        ...
        if item.Divider then
          ...
          if self.Iconless then
            ...
            -- Ignoran.MiscIcons:DisplayIgnoranIcons(newEntry.Icon, iconIndex)
  ...
  Lib.BrickColorPicker = (function()
    ...
    local function createGui(self)
      local gui = create({
        {1,"ScreenGui",{Name="BrickColor",Parent=game.Players.LocalPlayer:WaitForChild("PlayerGui"),}},
      ...
      if table.find(self.AllowedButtons, button) then
        ...
        task.spawn(function()
          if self.InputDown then
            ...
            while self.InputDown and not Ignoran.Dragging do
...
-- Main vars
local Main, Ignoran, Properties, ScriptViewer, DefaultSettings, Notebook, Serializer, Lib
local API, RM
...
Main = (function()
  ...
  Main.ModuleList = {"Ignoran", "Properties", "ScriptViewer"}
  Main.Elevated = false
  Main.MissingEnv = {}
  ...
  Main.Error = function(str)
    if rconsoleprint then
      rconsoleprint("Sun ERROR: "..tostring(str).."\n")
      wait(9e9)
    else
      error(str)
    end
  ...
  Main.InitEnv = function()
        ...
        Main.GuiHolder = Main.Elevated and service.Players.LocalPlayer.PlayerGui or plr:FindFirstChildWhichIsA("PlayerGui")

        setmetatable(env, nil)
    end

  Main.LoadSettings = function()
    local s,data = pcall(env.readfile or error,"SunSettings.json")
  ...
  Main.FetchAPI = function()
    ...
    if Main.Elevated then
      if Main.LocalDepsUpToDate() then
        local localAPI = Lib.ReadFile("sun/rbx_api.dat")
        if localAPI then 
          rawAPI = localAPI
        else
  ...
  Main.FetchRMD = function()
    ...
    if Main.Elevated then
      if Main.LocalDepsUpToDate() then
        local localRMD = Lib.ReadFile("sun/rbx_rmd.dat")
        if localRMD then 
          rawXML = localRMD
        else
          Main.DepsVersionData[1] = ""
  ...
  Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
    local gui = create({
      {1,"ScreenGui",{Name="Intro",Parent=game.Players.LocalPlayer:WaitForChild("PlayerGui"),}},
  ...
  Main.SetMainGuiOpen = function(val)
    ...
    Main.CreateMainGui = function()  
      local gui = create({
        {1,"ScreenGui",{Name="MainMenu",Parent=game.Players.LocalPlayer:WaitForChild("PlayerGui")}},
        ...
        {20,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="AppName",Parent={18},Position=UDim2.new(0,2,0,38),Size=UDim2.new(1,-4,1,-40),Text="Ignoran",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0.10000000149012,TextTruncate=1,TextWrapped=true,TextYAlignment=0,}},
    ...
    -- Create Main Apps
    Main.CreateApp({Name = "สำรวจไฟล์", IconMap = Main.LargeIcons, Icon = "Ignoran", Open = true, Window = Ignoran.Window})
  ...
  Main.SetupFilesystem = function()
    ...
    makefolder("sun")
    makefolder("sun/assets")
    makefolder("sun/saved")
    makefolder("sun/plugins")
    makefolder("sun/ModuleCache")
  end
  ...
  Main.Init = function()
    Main.Elevated = pcall(function() local a = service.Players.LocalPlayer.PlayerGui:GetFullName() end)
    Main.InitEnv()
    Main.LoadSettings()
    ...
    Main.LargeIcons:SetDict({
      Ignoran = 0, Properties = 1, Script_Viewer = 2,
    })
    
    -- Fetch version if needed
    intro.SetProgress("รอแปปกำลังโหลดเวอร์ชั่น roblox",0.2)
    if Main.Elevated then
      local fileVer = Lib.ReadFile("sun/deps_version.dat")
    ...
    if Main.Elevated and env.writefile and not Main.LocalDepsUpToDate() then
      env.writefile("sun/deps_version.dat",Main.ClientVersion.."\n"..Main.RobloxVersion)
      env.writefile("sun/rbx_api.dat",Main.RawAPI)
      env.writefile("sun/rbx_rmd.dat",Main.RawRMD)
    end
    ...
    -- Init window system, create main menu, show ignoran and properties
    Lib.Window.Init()
    Main.CreateMainGui()
    Ignoran.Window:Show({Align = "right", Pos = 1, Size = 0.5, Silent = true})
